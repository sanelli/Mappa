...
| [`PreventEnumerableCountMapper.cs`](PreventEnumerableCountMapper.cs) | `PreventEnumerableCount` avoids `Enumerable.Count` for `IEnumerable<T>` to fixed-size targets |
| [`EnumerableConcreteTypeMapper.cs`](EnumerableConcreteTypeMapper.cs) | `EnumerableConcreteType` list vs array buffers for sequence-like interface targets |
