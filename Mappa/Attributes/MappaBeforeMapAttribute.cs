// <copyright file="MappaBeforeMapAttribute.cs" company="Stefano Anelli">
...
/// <summary>
/// Instructs the Mappa source generator to invoke <see cref="MethodName"/> immediately before
/// executing the generated mapping body.
/// <para>
...
/// parameter, one <c>ref</c> parameter whose type exactly matches the mapping source type, or a matching
/// <c>ref</c> source parameter followed by <see cref="Mappa.MappaContext"/>. Context-aware hooks are
/// considered only when the mapping method provides a context. Candidate signatures are considered in
/// the following order: source and context, source only, context only, and no parameters.
/// </para>
/// <para>
/// Class-level before-map hooks execute before method-level before-map hooks. Declaration order is
/// preserved within each scope. If class and method scope resolve to the same before-map hook, the
/// generator reports a warning and invokes it once at the class-level position.
/// </para>
/// </summary>
[AttributeUsage(AttributeTargets.Method | AttributeTargets.Class, AllowMultiple = true)]
public sealed class MappaBeforeMapAttribute
    : Attribute
{
    /// <summary>
    /// Initializes a new instance of the <see cref="MappaBeforeMapAttribute"/> class.
    /// </summary>
    /// <param name="methodName">The name of the hook on the mapper class or an accessible base class.</param>
    public MappaBeforeMapAttribute(string methodName)
    {
    ...
    public MappaBeforeMapAttribute(Type classType, string methodName)
        : this(methodName)
    {
    ...
    public MappaBeforeMapAttribute(string fieldName, string methodName)
        : this(methodName)
    {
