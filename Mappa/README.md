...
- `MappaDependencyInjection`: Identifies a partial registrar class for which the [Mappa source generator](https://www.nuget.org/packages/Mappa.Generator/) emits a method that registers all `[Mappa]` mapper types from the same assembly into an `IServiceCollection` (see [MappaDependencyInjection](../Documentation/mappa-attributes.md#mappadependencyinjection));
