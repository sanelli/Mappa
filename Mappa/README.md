...
**Queryable projection:** map methods with signature `IQueryable<TSource>` → `IQueryable<TTarget>` are handled by the generator as deferred `Select` projections for ORM providers. This is signature-driven and does not require a new attribute in this package. See the [tutorial](../Documentation/tutorial.md#iqueryable-projection) and [IQueryableProjectionMapper.cs](../Mappa.Samples/IQueryableProjectionMapper.cs).
