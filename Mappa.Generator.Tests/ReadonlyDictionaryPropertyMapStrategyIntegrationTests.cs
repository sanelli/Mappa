    ...
    : MappaGeneratorAbstractUnitTests
{
    ...
    /// Test that a map from <see cref="Dictionary{TKey,TValue}"/> where key is <see cref="int"/> and value is <see cref="string"/>
    /// <see cref="Dictionary{TKey,TValue}"/> where key is <see cref="string"/> and value is <see cref="int"/>
    /// and the target property has a private setter.
    /// </summary>
    /// <returns>The async task.</returns>
    [Fact]
    [IntegrationTest]
    public async Task CanMapDictionaryToDictionaryWhenSetterIsPrivateOnTargetProperty()
    {
        // Arrange
        const string sourceCode = """
                                  #nullable enable
                                  ...
                                  public class Target
                                  {
                                      public Dictionary<string, int> PropertyA {get; private set;}
                                  }
    ...
    /// <summary>
    /// Test that a map from <see cref="Dictionary{TKey,TValue}"/> where key is <see cref="int"/> and value is <see cref="string"/>
    /// custom non-generic type implementing explicitly <see cref="Dictionary{TKey,TValue}"/> where
    /// key is <see cref="string"/> and value is <see cref="int"/>
    /// and the target property does not have a setter.
    /// </summary>
    /// <returns>The async task.</returns>
    [Fact]
    [IntegrationTest]
    public async Task CanMapDictionaryToDictionaryWhenSetterIsNotProvidedOnTargetPropertyAndTargetIsNonGenericExplicitImplementation()
    {
        // Arrange
        const string sourceCode = """
                                  #nullable enable
                                  ...
                                  public partial class TargetDictionary : IDictionary<string, int>
                                  {
                                  ...
                                  public class Target
                                  {
                                      public TargetDictionary PropertyA {get;}
                                  }
                                ...
                                forBlockAssertions =>
                                {
                                    forBlockAssertions
                                        ...
                                        .HasNextSyntaxNode(forSyntaxStatement =>
                                        {
                                            forSyntaxStatement
                                                .BeAssignmentExpressionStatement(
                                                    leftExpression => leftExpression.BeElementAccessExpressionSyntaxWithIdentifierNameSyntax(
                                                        "__mappa_tmp_8",
                                                        "__mappa_tmp_5"),
                                                    rightExpression => rightExpression.BeIdentifierNameSyntax("__mappa_tmp_7"));
                                        });
    ...
    /// <summary>
    /// Test that a map from <see cref="Dictionary{TKey,TValue}"/> where key is <see cref="int"/> and value is <see cref="string"/>
    /// custom generic type implementing explicitly <see cref="Dictionary{TKey,TValue}"/> where
    /// key is <see cref="string"/> and value is <see cref="int"/>
    /// and the target property does not have a setter.
    /// </summary>
    /// <returns>The async task.</returns>
    [Fact]
    [IntegrationTest]
    public async Task CanMapDictionaryToDictionaryWhenSetterIsNotProvidedOnTargetPropertyAndTargetIsGenericExplicitImplementation()
    {
        // Arrange
        const string sourceCode = """
                                  #nullable enable
