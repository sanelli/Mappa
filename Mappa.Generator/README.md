...
6. <u>`string` strategy</u>:
    ...
    - _What_:
      - `TSource` is a `string` and `TTarget` is any of the numeric types then the relevant `Parse` method will be invoked, possibly with the culture and [NumberStyles](https://learn.microsoft.com/en-us/dotnet/api/system.globalization.numberstyles) identified by the `MappaSettings` attribute, if any is provided on the class or on the method;
      - When parsing numeric types from `string`, if a style is configured the generator emits the BCL overload that accepts `NumberStyles`: `Parse(input, style)` when culture is not configured, or `Parse(input, style, culture)` when culture is configured. When style is not configured, the existing overload without `NumberStyles` is used unchanged;
       - `TSource` is a `string` and `TTarget` is any of the following types `DateTime`, `DateTimeOffset`, `DateOnly`, `TimeOnly`, `Guid` then their `TTarget.Parse` or `TTarget.ParseExact` method will be used, possibly with the format, culture, and [DateTimeStyles](https://learn.microsoft.com/en-us/dotnet/api/system.globalization.datetimestyles) identified by the `MappaSettings` attribute, if any is provided on the class or on the method;
       - When parsing `DateTime`, `DateTimeOffset`, `DateOnly`, or `TimeOnly` from `string`, if a style is configured the generator emits the BCL overload that accepts `DateTimeStyles` as the last parameter: `Parse(input, culture, style)` or `ParseExact(input, format, culture, style)`. When culture is not configured, `null` is passed for `IFormatProvider`. When style is not configured, the existing overload without `DateTimeStyles` is used unchanged;
